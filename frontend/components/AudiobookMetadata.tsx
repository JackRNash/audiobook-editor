...
export default function AudiobookMetadata({
  ...
  const handleAuthorEdit = () => {
    ...
    setIsEditingAuthor(!isEditingAuthor)
  }
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6 mb-6 flex items-start space-x-6">
      <div className="relative w-40 h-40 bg-gray-700 rounded-lg overflow-hidden">
        {coverUrl ? (
          <img src={coverUrl || "/placeholder.svg"} alt="Audiobook cover" className="w-full h-full object-cover" />
        ) : (
        ...
        <div className="mb-4">
          {isEditingTitle ? (
            ...
            <h2 className="text-2xl font-bold flex items-center">
              {title}
              <Button size="sm" variant="ghost" onClick={() => setIsEditingTitle(true)} className="ml-2">
        ...
        <div>
          {isEditingAuthor ? (
            ...
            <p className="text-lg text-gray-300 flex items-center">
              {author}
              <Button size="sm" variant="ghost" onClick={() => setIsEditingAuthor(true)} className="ml-2">
                <Pencil className="w-4 h-4" />
              </Button>
            </p>
          )}
