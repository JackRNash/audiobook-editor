...
import { Play, Pause, SkipForward, SkipBack, Pencil, Trash2, Check, X, Rewind, FastForward } from "lucide-react"
...
export default function AudioPlayer({
  ...
  const skipToChapter = (direction: "forward" | "backward") => {
    ...
    if (direction === "forward") {
      newIndex = currentChapterIndex === -1 ? 0 : Math.min(currentChapterIndex + 1, chapters.length - 1)
    } else {
      newIndex = currentChapterIndex === -1 ? chapters.length - 1 : Math.max(currentChapterIndex - 1, 0)
    }
      ...
      {currentChapter && (
        <div className="flex items-center justify-between text-lg font-semibold text-white bg-gray-600 rounded-lg p-2">
          ...
          <div className="flex space-x-2">
            {isEditing ? (
              <>
                <Button size="sm" variant="ghost" onClick={handleSaveEdit} className="hover:bg-gray-500">
                  <Check className="h-4 w-4" />
                </Button>
                <Button size="sm" variant="ghost" onClick={handleCancelEdit} className="hover:bg-gray-500">
                  <X className="h-4 w-4" />
                </Button>
              </>
            ) : (
              <>
                <Button size="sm" variant="ghost" onClick={handleEditClick} className="hover:bg-gray-500">
